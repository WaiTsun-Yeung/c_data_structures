...
int main(){
    ...
    int32_t* const values = malloc(values_count * sizeof(int32_t));
    int32_t* values_initialise_ptr = values;
    ...
    *values_initialise_ptr++ = -26632;
    ...
    struct cds_array* array
        = cds_create_int32_t_array(
            &return_state, values_count, 
            24065, 22311, 24949, 8873, -21217, -26215, 27941, 27079, 22208, 
            -26632
        );
    for (
        const int32_t* array_ptr = cds_data(array), *values_ptr = values;
        array_ptr < (int32_t*)cds_data(array) + array->elements_count;
